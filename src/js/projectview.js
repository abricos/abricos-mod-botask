...
Component.entryPoint = function(NS){
	...
	ProjectViewWidget.prototype = {
		...
		renderTask: function(){
			var task = this.task;
			var TM = this._TM, gel = function(nm){ return TM.getEl('panel.'+nm); };
			...
			switch(task.status){
			case TST.OPEN:
			case TST.REOPEN:	TM.elShow('panel.bremove'); break;
			case TST.ACCEPT:	TM.elShow('panel.bremove'); break;
		...
		onClick: function(el){
			...
			case tp['fav']: 
			case tp['favi']: 
				this.taskFavorite(); return true;

			case tp['bremove']:  this.taskRemove(); return true;
			case tp['brestore']: this.taskRestore(); return true;
			case tp['barhive']: this.taskArhive(); return true;
	...
	YAHOO.extend(ProjectRemovePanel, Brick.widget.Dialog, {
		...
		taskRemove: function(){
			var TM = this._TM, gel = function(n){ return  TM.getEl('tkremovepanel.'+n); },
				__self = this;
