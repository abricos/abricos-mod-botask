...
Component.entryPoint = function(NS){
	...
	CustatusSelectWidget.prototype = {
		...
		render: function(){
			...
			for (var tid in st){
				...
				if (ph.length == 0 || dict[ph]){ continue; }
				dict[ph] = true;
		...
		chickTask: function(tk){
			...
			var st = this.ustat;
			for (var tid in st){
				if (tk.id == tid && st[tid] == ph){ return true; }
			}
	...
	FilterWidget.prototype = {
		init: function(container, sts){
			...
			var TM = buildTemplate(this, 'widget'), gel = function(n){ return TM.getEl('widget.'+n);};
		...
		onClick: function(el){
			...
			switch(el.id){
			case tp['bsetfilter']:
				this.listWidget.render(); return true;
			}
